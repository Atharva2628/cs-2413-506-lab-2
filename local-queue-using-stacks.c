...
int main() {
    MyQueue* queue_object = myQueueCreate();

    //queue elements: 10, 20, 30
    myQueuePush(queue_object, 10);
    myQueuePush(queue_object, 20);
    myQueuePush(queue_object, 30);
    
    myQueuePeek(queue_object);                      
    
    myQueuePop(queue_object);

    myQueuePeek(queue_object);

    myQueueEmpty(queue_object);                     
    ...
    //checking pop and peek for empty queue (underflow)
    myQueuePop(queue_object);           
    myQueuePeek(queue_object);
    
    myQueueFree(queue_object);   
    return 0;
}